...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="/dbconf?p=dynv6_&v=<% uptime(); %>"></script>
		...
		function init() {
		    ...
		    if (document.form.dynv6_enable.value != "1") {
		        rrt.checked = false;
		        document.getElementById('dynv6_detail_table').style.display = "none";
		    } else {
		        rrt.checked = true;
		        document.getElementById('dynv6_detail_table').style.display = "";
		    }
			//双wan开始判断
			...
			if(lb_mode !== "lb"){
				...
				document.form.dynv6_config_wan.value = 0;
			} else {
				check_selected("dynv6_config_wan", db_dynv6_.dynv6_config_wan);
			}
			version_show();
			write_dynv6_run_status();
		...
		function onSubmitCtrl(o, s) {
			...
			//开始赋值
			var pwd = $("#dynv6_config_old_pwd").val();
			var encrypted_pwd = rsa.encrypt(md5(pwd));
			$("#dynv6_config_pwd").val(encrypted_pwd.toUpperCase());
			showLoading(9);
			...
			$.ajax({
				...
				success: function(xhr) {
			        ...
			        for (var i = 0; i < params.length; i++) {
						if (typeof db_dynv6_[p + params[i]] !== "undefined") {
							$("#dynv6_"+params[i]).val(db_dynv6_[p + params[i]]);
						}
			        }
                    $("#dynv6_run_state").html(db_dynv6_['dynv6_run_status']);
					check_selected("dynv6_auto_start", db_dynv6_.dynv6_auto_start);
					check_selected("dynv6_delay_time", db_dynv6_.dynv6_delay_time);
					check_selected("dynv6_refresh_time", db_dynv6_.dynv6_refresh_time);
				}
		...
		function buildswitch(){
			...
			function(){
				if(document.getElementById('switch').checked){
					document.form.dynv6_enable.value = 1;
					document.getElementById('dynv6_detail_table').style.display = "";
				}else{
					document.form.dynv6_enable.value = 0;
					document.getElementById('dynv6_detail_table').style.display = "none";
				}
			});
		}
		...
		function write_dynv6_run_status(){
			$.ajax({
				type: "get",
				url: "dbconf?p=dynv6_",
				dataType: "script",
				success: function() {
					...
					setTimeout("write_dynv6_run_status()", 10000);
				}
			});
		}

		function version_show(){
			$("#dynv6_version_status").html("<i>当前版本：" + db_dynv6_['dynv6_version']);
		    $.ajax({
		        ...
		        success: function(res) {
		            ...
		            if(typeof(txt) != "undefined" && txt.length > 0) {
				        ...
				        $("#dynv6_version_status").html("<i>当前版本：" + obj.version);
				        if(obj.version != db_dynv6_["dynv6_version"]) {
					        $("#dynv6_version_status").html("<i>有新版本：" + obj.version);
				        }
		            }
		        }
		    });
    ...
    <body onload="init();">
		...
		<form method="post" name="form" action="/applydb.cgi?p=dynv6_" target="hidden_frame">
			<input type="hidden" name="current_page" value="Module_dynv6.asp"/>
			<input type="hidden" name="next_page" value="Module_dynv6.asp"/>
			...
			<input type="hidden" name="SystemCmd" onkeydown="onSubmitCtrl(this, ' Refresh ')" value="dynv6_config.sh"/>
			<input type="hidden" name="firmver" value="<% nvram_get("firmver"); %>"/>
			<input type="hidden" id="dynv6_config_pwd" name="dynv6_config_pwd" value='<% dbus_get_def("dynv6_config_pwd", ""); %>'/>
			<input type="hidden" id="dynv6_enable" name="dynv6_enable" value='<% dbus_get_def("dynv6_enable", "0"); %>'/>
									...
									<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3" class="FormTitle" id="FormTitle">
										<tr>
											<td bgcolor="#4D595D" colspan="3" valign="top">
												<div>&nbsp;</div>
												<div style="float:left;" class="formfonttitle">Dynv6</div>
                                                ...
                                                <div>使用dynv6实现的ddns服务</div>
                                                <ul style="padding-top:5px;margin-top:10px;float: left;">
                                                <li>使用前需要将域名添加到dynv6中，并添加主机分别为“@”和“*”的A记录、和AAAA记录，使用之后将自动更新ipv4和ipv6</li>
                                                <li>使用dynv6 Token，可以保护账户安全</li>
                                                <li>点 <a href="https://dynv6.com/"><i><u>这里</u></i></a> 获取dynv6 Token</li>
                                                </ul>
                                                </div>
												...
												<table style="margin:10px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" id="dynv6_detail_table">
													...
													<tr>
													    <th width="35%">开机自启</th>
														<td>
															<select id="dynv6_auto_start" name="dynv6_auto_start" class="input_option"  >
													...
													<tr id="select_wan">
														...
														<td>
															<select id="dynv6_config_wan" name="dynv6_config_wan" class="input_option"  >
